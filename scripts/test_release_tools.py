...
def package_fixtures(root: Path) -> tuple[Path, Path, Path, Path]:
    ...
    npm_platform = root / "dslzl-csa-win32-x64-0.1.3.tgz"
    write_tar(
        npm_platform,
        {
            "package/LICENSE": b"fixture license\n",
...
def test_assembler(root: Path) -> None:
    ...
    assert digest(first / "source" / "csa-0.1.3.tar.gz") == digest(
        second / "source" / "csa-0.1.3.tar.gz"
    )
